...
const getSuggestions = async (materials, difficulty, language) => {
...
Do not suggest projects that require materials not on the list. You can assume smaller quantities of the provided materials can be used.`;
  ...
  const suggestionsWithoutImages = parseJsonResponse(responseText);
...
const getSteps = async (suggestion, language) => {
...
`;
    ...
    const projectData = parseJsonResponse(responseText);
...
const Icons = {
  ...
  Power: (className) => `<svg xmlns="http://www.w3.org/2000/svg" class="${className || 'h-5 w-5'}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>`,
  ...
  Bot: (small) => `<svg xmlns="http://www.w3.org/2000/svg" class="${small ? 'h-4 w-4' : 'h-6 w-6'}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 8V4H8"/><rect width="16" height="12" x="4" y="8" rx="2"/><path d="M2 14h2"/><path d="M20 14h2"/><path d="M15 13v2"/><path d="M9 13v2"/></svg>`,
...
const renderMaterialInputManager = () => {
